...
function renderForecast(card, data) {
  ...
  card.querySelector('.main .humidity')
      .textContent = Math.round(data.main.humidity * 100)
      console.log(Math.round(data.main.humidity * 100));
...
function getForecastFromNetwork(coords) {
  return fetch(`http://api.openweathermap.org/data/2.5/weather?q=${coords}&appid=938b1db46eb145cc6a6a50b339e9940a`)
      .then((response) => {
...
function loadLocationList() {
  ...
  if (!locations || Object.keys(locations).length === 0) {
    const key = 'lagos';
    locations = {};
    locations[key] = {label: 'Lagos', geo: 'lagos'};
  }
  return locations;
}
